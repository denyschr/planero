...
describe(Register.name, () => {
  function setup() {
    const userApiClientSpy = jasmine.createSpyObj<UserApiClient>('UserApiClient', ['register']);
    ...
    const fakeCredentials = { username: 'foo', email: 'foo@gmail.com', password: '12345678' };
  ...
  it('should have a prompt text with the link to the login page', () => {
    const { element } = setup();
    const text = element.querySelector('p#login-prompt-text')!;

    expect(text)
      .withContext('You should have a `p` element to display the prompt text')
      .toBeTruthy();
    expect(text.textContent).toContain('Have an account?');
    expect(text.textContent)
      .withContext('You should have the `a` element inside the `p` element')
      .toContain('Sign in');

    const link = text.querySelector('a[href="/login"]')!;
    expect(link)
      .withContext('You should have an `a` element to display the link to the login page')
      .toBeTruthy();
    expect(link.textContent).withContext('The link should have a text').toContain('Sign in');
  });
  ...
  it('should be possible to register if the form is complete', () => {
    ...
    expect(usernameInput).withContext('You should have an input for the username').toBeTruthy();
    usernameInput.value = fakeCredentials.username;
  ...
  it('should display validation error messages if fields are dirty and invalid', () => {
    ...
    const usernameRequiredError = element.querySelector('#username-required-error')!;
    expect(usernameRequiredError)
      .withContext('You should have an error message if the username field is required and dirty')
      .toBeTruthy();
  ...
  it('should call the user api client and redirect on success', () => {
    ...
    userApiClientSpy.register.and.returnValue(of({ id: '1' } as User));
    ...
    expect(usernameInput.hasAttribute('disabled'))
      .withContext('Your username field should be disabled when the form is submitted')
      .toBeTruthy();
    ...
    expect(button.hasAttribute('disabled'))
      ...
      .toBeTruthy();
    expect(userApiClientSpy.register).toHaveBeenCalledOnceWith(fakeCredentials);
    expect(router.navigateByUrl).toHaveBeenCalledOnceWith('/');
  });

  it('should call the user api client and display a message on failure', () => {
    const { fixture, element, userApiClientSpy, router, fakeCredentials } = setup();
    userApiClientSpy.register.and.callFake(() => throwError(() => new Error('error')));
    ...
    expect(usernameInput.hasAttribute('disabled'))
      .withContext('Your username field should NOT be disabled when the form submission fails')
      .toBeFalsy();
    expect(emailInput.hasAttribute('disabled'))
    ...
    const message = element.querySelector('#registration-failed-message')!;
    expect(message)
      .withContext('You should display an error message if the registration fails')
      .toBeTruthy();
    expect(message.textContent).toContain('Try again with another username or email');
  });
});
